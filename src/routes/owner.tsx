...
function OwnerDashboard() {
  ...
  async function load() {
    if (!isConfigured()) { setError("Lead tracking is not configured for this deployment."); setLoading(false); return; }
    setLoading(true); setError("");
    const { data: sessionData } = await supabase!.auth.getSession();
    if (!sessionData.session) { void navigate({ to: "/owner/login", replace: true }); return; }
    ...
    if (leadResult.error) setError("Could not load leads."); else setLeads((leadResult.data || []) as Lead[]);
  ...
  const filtered = useMemo(() => leads.filter(l => {
    ...
    const matchesDate = dateFilter === "All" || (dateFilter === "Today" ? new Date(l.created_at).toDateString() === new Date().toDateString() : dateFilter === "This Week" ? inRange(l.created_at, 7) : inRange(l.created_at, 30));
  ...
  const counts = useMemo(() => ({
    ...
    booked: leads.filter(l => l.lead_status === "Assessment Booked").length,
    Completed: leads.filter(l => l.lead_status === "Completed").length,
    ...
    week: leads.filter(l => inRange(l.created_at, 7)).length,
    month: leads.filter(l => inRange(l.created_at, 30)).length,
  }), [leads]);
  ...
  return <main className="min-h-screen bg-background text-charcoal">
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
      ...
      <p className="mt-4 text-xs text-muted-foreground">All figures are calculated from the existing LiveFit lead database and tracking events.</p>
    </div>
