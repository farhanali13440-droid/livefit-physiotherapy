...
function getStoredAttribution(): Partial<Attribution> | null {
  ...
  try {
    return JSON.parse(window.localStorage.getItem("livefit_attribution") || "null");
  } catch {
    return null;
  }
}
...
function getAttribution(): Attribution {
  ...
  if (utmSource) {
    source = ["google", "googleads", "adwords"].includes(utmSource)
      ? "Google Ads"
      : ["meta", "facebook", "instagram"].includes(utmSource)
        ? "Meta Ads"
        : utmSource;
  } else if (gclid) {
    source = "Google Ads";
  } else if (params.get("fbclid") || /facebook\.com|instagram\.com/i.test(referrerHost)) {
    source = "Meta Ads";
...
export async function trackLeadEvent(
  ...
  page: string,
  ctaLocation: string,
  options?: { leadId?: string | null; contactMethod?: "call" | "whatsapp"; eventData?: Record<string, unknown> },
) {
  if (!supabase) return;
  ...
  const { error } = await supabase.from("lead_events").insert({
    ...
    contact_method: options?.contactMethod || null,
...
export async function updateLeadContactMethod(
  leadId: string,
  contactMethod: "Call" | "WhatsApp",
) {
  if (!supabase) throw new Error("Lead tracking is not configured yet.");
  const { data, error } = await supabase.rpc("update_livefit_lead_contact_method", {
    p_lead_id: leadId,
