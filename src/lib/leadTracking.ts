...
type Attribution = Record<(typeof ATTRIBUTION_KEYS)[number], string | null> & { landing_page:string; referrer:string|null; source:string|null; gclid:string|null; fbclid:string|null };
...
function getAttribution(): Attribution {
  if (typeof window === "undefined") return {utm_source:null,utm_medium:null,utm_campaign:null,utm_content:null,utm_term:null,landing_page:"",referrer:null,source:null,gclid:null,fbclid:null};
  ...
  const referrer = stored?.referrer || document.referrer || null;
  ...
  const attribution: Attribution = {utm_source:value("utm_source"),utm_medium:value("utm_medium"),utm_campaign:value("utm_campaign"),utm_content:value("utm_content"),utm_term:value("utm_term"),gclid,fbclid,landing_page:stored?.landing_page || `${window.location.origin}${window.location.pathname}`,referrer,source};
